...
**Scope:** Next.js App Router. `src/app/layout.tsx` (fonts, Providers, Header), `src/app/page.tsx` (single page: stats, DataImport, RankingControls, LeadsTable, ExportButton). Components in `src/components/`; tRPC client and React Query in `src/utils/trpc.ts` using `NEXT_PUBLIC_SERVER_URL`. UI uses shadcn-style components under `src/components/ui/`.
