...
const ProgressBar = ({ currentCompany, completed, total, percent }: ProgressBarProps) => (
  <div className="space-y-2">
    <div className="flex justify-between text-sm">
      <span>{currentCompany ? `Processing: ${currentCompany}` : "Starting..."}</span>
...
const NoProviderWarning = () => (
  <div className="rounded-md bg-yellow-500/10 border border-yellow-500/20 p-3 text-sm text-yellow-600 dark:text-yellow-400">
    No AI provider configured. Please add OPENAI_API_KEY, ANTHROPIC_API_KEY, or OPENROUTER_API_KEY to your environment.
  </div>
);
...
export function RankingControls() {
  ...
  const progressPercent = calculateProgressPercent(progress?.completed ?? 0, progress?.total ?? 0);
