	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Use genetic algorithms to automatically optimize the ranking prompt
			...
			<CardContent className="space-y-6">
				...
				{evalLoading ? (
					...
					<div className="rounded-md bg-muted p-4">
						<h4 className="mb-2 font-medium">Evaluation Dataset</h4>
					...
					<div className="rounded-md border border-yellow-500/20 bg-yellow-500/10 p-3 text-sm text-yellow-600 dark:text-yellow-400">
						Evaluation dataset not found. Please ensure eval_set.csv exists in
						the project root.
					</div>
				)}
				...
				{isRunning && progress && (
					<div className="space-y-3">
						<div className="flex justify-between text-sm">
							<span>
								Generation {progress.currentGeneration} of{" "}
						...
						<div className="text-sm">
							...
							<span className="text-green-600">
								{(progress.bestFitness * 100).toFixed(1)}%
							</span>
						</div>
					</div>
				)}
								...
								>
									...
									{!prompt.isActive && (
										<Button
											...
											size="sm"
											onClick={() =>
												activatePrompt.mutate({ version: prompt.version })
											}
