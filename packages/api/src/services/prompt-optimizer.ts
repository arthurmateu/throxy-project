...
const scorePrediction = (predicted: number | null, actual: number | null): number => {
...
const buildEvalPrompt = (promptContent: string, company: string, companyLeads: EvalLead[]): string => {
...
const findLeadByName = (leads: EvalLead[], name: string): EvalLead | undefined =>
  leads.find((l) => l.fullName.toLowerCase() === name?.toLowerCase());

/** Parse ranking results from response */
const parseEvalResponse = (content: string, companyLeads: EvalLead[]): Prediction[] => {
  const predictions: Prediction[] = [];
  const processedNames = new Set<string>();

  const parsed = extractJsonFromResponse(content);
  if (parsed && Array.isArray((parsed as { rankings?: unknown[] }).rankings)) {
    for (const result of (parsed as { rankings: Array<{ name?: string; rank?: number | null }> }).rankings) {
...
const logAiCall = async (
  provider: AIProvider,
  response: { model: string; inputTokens: number; outputTokens: number; cost: number; durationMs: number },
  batchId: string
): Promise<void> => {
...
const evaluatePrompt = async (
  ...
  for (const [company, companyLeads] of companiesMap.entries()) {
    const companyPredictions = await evaluateCompanyBatch(promptContent, company, companyLeads, provider, runId);
...
const tournamentSelect = (population: PromptCandidate[], tournamentSize: number): PromptCandidate => {
...
const buildMutationPrompt = (parentPrompt: string, errorPatterns: string[]): string =>
...
const mutatePrompt = async (
  ...
  const response = await ai.chat(provider, [{ role: "user", content: prompt }], {
    temperature: 0.7,
    maxTokens: 4000,
  });
...
const initializePopulation = async (
  ...
  const initialErrors = ["- Initial evaluation: creating diverse variations to explore the solution space."];

  const population: PromptCandidate[] = [createCandidate(baseContent, baseVersion, 0)];

  let nextVersion = baseVersion + 1;

  for (let i = 1; i < populationSize; i++) {
    const mutated = await mutatePrompt(baseContent, initialErrors, provider, runId);
...
const evaluatePopulation = async (
  ...
  for (let i = startFrom; i < population.length; i++) {
    ...
    const { fitness } = await evaluatePrompt(candidate.content, evalLeads, provider, runId);
...
const generateOffspring = async (
  ...
  return {
    candidate: createCandidate(childContent, version, generation, parent1.version),
    extraEvaluations,
  };
};
...
const runGeneration = async (
  ...
  nextVersion: number,
  initialErrors: string[]
): Promise<{ newPopulation: PromptCandidate[]; nextVersion: number; evaluationsRun: number }> => {
  ...
  evaluationsRun += await evaluatePopulation(newPopulation, sampledLeads, provider, runId, eliteCount);
...
export const runPromptOptimization = async (
  ...
  try {
    ...
    let totalEvaluations = await evaluatePopulation(population, sampledLeads, provider, runId);
    ...
    for (let gen = 1; gen <= generations; gen++) {
      ...
      console.log(`Generation ${gen}: Best fitness = ${bestCandidate.fitness.toFixed(3)}`);
    }
